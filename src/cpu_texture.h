...
inline fvec4 pixel_to_float(pixel sample) {
    return fvec4(static_cast<float>(sample[0]), static_cast<float>(sample[1]), static_cast<float>(sample[2]),
...
inline sdr_pixel float_pixel_to_srgb8(hdr_pixel sample) {
    ...
    return sdr_pixel{
        static_cast<uint8_t>(out.x), static_cast<uint8_t>(out.y), static_cast<uint8_t>(out.z), static_cast<uint8_t>(out.w)};
}
...
static inline fvec4 pixel_srgb8_to_linear(const sdr_pixel p) {
    return fvec4{srgb8_to_linear(p[0]), srgb8_to_linear(p[1]), srgb8_to_linear(p[2]), static_cast<float>(p[3]) / 255.0f};
}
