...
static ray_triangle_hit_info NaiveAS::mesh_ray_intersection(
    ...
    ray os_ray{ray_origin_os, normalize(ray_direction_os)};
...
BVH_AS::MeshBVHData::MeshBVHData(const Mesh& mesh, bool double_sided, unsigned int max_triangles_per_leaf)
    ...
    for (size_t i = 0; i < mesh.indices.size(); i += 3) {
        MeshBVHNode::triangle tri{mesh.vertices[mesh.indices[i]].position, mesh.vertices[mesh.indices[i + 1]].position,
            mesh.vertices[mesh.indices[i + 2]].position, static_cast<uint32_t>(i)};
...
ray_triangle_hit_info BVH_AS::intersect_ray(const ray& ray, bool any_hit) const {
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        ray_projections[i] = fvec2{dot(ray.origin, DOP::axises[i]), 1.0f / dot(ray.direction, DOP::axises[i])};
    }
